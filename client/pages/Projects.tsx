...
type Tech = "React" | "TypeScript" | "Tailwind" | "Node.js" | "IA" | "Automação";
...
const filters: (typeof ALL_TAG | Tech)[] = [ALL_TAG, "React", "TypeScript", "Tailwind", "Node.js", "IA", "Automação"];
      ...
      <section className="relative isolate overflow-hidden">
        ...
        <div className="container py-16">
          ...
          <p className="mt-2 max-w-2xl text-white/70">
            Seleção de trabalhos e provas de conceito com foco em performance e experiência.
          </p>
        </div>
      </section>
            ...
            >
              ...
              <div className="p-5">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {p.tech.map((t) => (
                    <span key={t} className={`inline-flex items-center rounded-full bg-gradient-to-r px-2.5 py-1 text-xs font-semibold ${techPalette[t]}`}>
                      {t}
                    </span>
                  ))}
                </div>
                <div className="mt-4 flex gap-2">
                  <a href={p.codeUrl ?? "#"} target="_blank" rel="noreferrer" className="rounded-md border border-white/15 bg-white/5 px-4 py-2 text-sm font-semibold text-white/90 transition hover:bg-white/10 hover:text-white">
                    Ver Código
                  </a>
                  <a href={p.demoUrl ?? "#"} target="_blank" rel="noreferrer" className="rounded-md bg-gradient-to-r from-indigo-500 to-fuchsia-500 px-4 py-2 text-sm font-semibold text-white transition hover:brightness-110">
                    Abrir Demo
                  </a>
                </div>
              </div>
